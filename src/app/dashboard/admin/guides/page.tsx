  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24, gap: 12, flexWrap: 'wrap' as const }}>
        <div>
          <h1 style={{ fontFamily: 'Montserrat', fontWeight: 800, fontSize: 22, marginBottom: 4, color: 'var(--text-primary)' }}>Guides</h1>
          <p style={{ fontSize: 14, color: 'var(--text-secondary)' }}>Published posts appear at /guides/[slug] and are indexed by Google.</p>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 10 }}>
          {posts.map(p => (
            <div key={p.id} style={{ background: 'var(--bg-card)', border: '1px solid var(--border-color)', borderRadius: 12, padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' as const }}>
              <div style={{ flex: 1, minWidth: 200 }}>
                ...
                <div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>/guides/{p.slug} · {p.is_published ? '✅ Published' : '📖 Draft'} · {p.author}</div>
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                {p.is_published && (
                  <a href={`${SITE_URL}/guides/${p.slug}`} target="_blank" rel="noreferrer"
          ...
          {posts.length === 0 && (
            <div style={{ textAlign: 'center' as const, padding: 60, color: 'var(--text-secondary)' }}>
              ...
              <div style={{ fontFamily: 'Montserrat', fontWeight: 700, fontSize: 16 }}>No guides yet</div>
            </div>
          )}
        </div>
      )}
        ...
        <div style={{ position: 'fixed' as const, inset: 0, background: 'rgba(0,0,0,0.65)', zIndex: 1000, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: 20, overflowY: 'auto' as const }}>
          <div style={{ background: '#f5f5f3', borderRadius: 16, width: '100%', maxWidth: 680, margin: '40px auto', boxShadow: '0 24px 60px rgba(0,0,0,0.4)' }}>
            <div style={{ padding: '20px 24px', background: 'white', borderRadius: '16px 16px 0 0', borderBottom: '1px solid #e8e8e5', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h2 style={{ fontFamily: 'Montserrat', fontWeight: 700, fontSize: 18, color: '#1A1A1A' }}>{editing ? 'Edit Post' : 'New Guide'}</h2>
            ...
            <div style={{ padding: 24, maxHeight: '75vh', overflowY: 'auto' as const, display: 'flex', flexDirection: 'column' as const, gap: 14 }}>
              ...
              <div>
                <label style={{ fontSize: 11, fontWeight: 600, display: 'block', marginBottom: 6, color: '#444', textTransform: 'uppercase' as const, letterSpacing: '0.06em' }}>Slug * ({SITE_URL}/guides/<strong>{form.slug || 'slug'}</strong>)</label>
              ...
              <div>
                ...
                <textarea value={form.excerpt} onChange={e => setF('excerpt', e.target.value)} style={{ ...inp, minHeight: 72, resize: 'vertical' as const }} placeholder="Short summary shown in guide listings..." />
              </div>
              <div>
                ...
                <textarea value={form.body} onChange={e => setF('body', e.target.value)} style={{ ...inp, minHeight: 200, resize: 'vertical' as const }} placeholder="<p>Full guide content...</p>" />
              </div>
              <div>
