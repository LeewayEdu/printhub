...
import { LayoutDashboard, ShoppingBag, User, LogOut, Package, Menu, X, ChevronRight, MessageSquare, TrendingUp, Truck, Tag, Bell, Image, DollarSign, Users, Zap, Star, FileSpreadsheet, Heart, AlignJustify, Calculator } from 'lucide-react'
...
const superAdminLinks: NavLink[] = [
  ...
  { href: '/dashboard/admin/trust-ticker', label: 'Trust Ticker', icon: AlignJustify },
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const links: NavLink[] = role === 'super_admin' ? superAdminLinks
        ...
        {profile && (
          <div style={{ padding: '16px 24px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
            <div style={{ width: 40, height: 40, borderRadius: '50%', background: 'var(--red)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 10 }}>
              <span style={{ fontFamily: 'Montserrat', fontWeight: 700, fontSize: 16, color: 'white' }}>
                {profile.first_name?.[0]?.toUpperCase() || 'U'}
              </span>
            </div>
            <div style={{ fontFamily: 'Montserrat', fontWeight: 600, fontSize: 14, color: 'white' }}>
